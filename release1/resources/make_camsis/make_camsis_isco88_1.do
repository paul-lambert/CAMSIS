...
** ALGORITHM FOR AUTOMATED SCALE GENERATION FOR ISCO88 4-DIGIT UNITS 
...
******   is88_ca socc focc usa_cam1 1 1 
...
**  as diagonals, we would call the programme as: 
******   is88_ca hjob wjob rom_cs freq 0 
...
capture program drop is88_ca

program define is88_ca 
...
** 4digit level recodes (usually redundant) 
...
sort `occ1' 
correlate rawsc1 rawsc2 rawsc3 rawsc4 occ1s `occ1' 
correlate rawsc1 `occ1' 
capture scalar drop c1
scalar c1=r(rho)
correlate rawsc2 `occ1'
...
use $path9\ca.dta, clear 
sort `occ2' 
correlate rawsc3 rawsc4 occ2s `occ2'
correlate rawsc3 `occ2'
capture scalar drop c1
scalar c1=r(rho)
correlate rawsc4 `occ2'
capture scalar drop c2
scalar c2=r(rho)
capture drop dimused
gen dimused=1
...
**  Next, distribute scores to all known iscos, with the 'orig' indicator to show if isco was represented in 
...
** In the special case of isco, it is also possible to have 1, 2 and 3 digit occs (e.g. 1, 13)
** which are equivalent to the 4-digit major group summaries derived above (e.g. 1000, 1300) 
replace occ3r=$occtype if $occtype >= 100 & $occtype <= 999 
replace occ3r=($occtype)*10 if $occtype >= 10 & $occtype <= 99 
replace occ3r=($occtype)*100 if $occtype >= 1 & $occtype <= 9
replace occ2r=floor($occtype/10) if $occtype >= 100 & $occtype <= 999
replace occ2r=$occtype if $occtype >= 10 & $occtype <= 99 
replace occ2r=($occtype)*10 if $occtype >= 1 & $occtype <= 9 
replace occ1r=floor($occtype/100) if $occtype >= 100 & $occtype <= 999
replace occ1r=floor($occtype/10) if $occtype >= 10 & $occtype <= 99 
replace occ1r=$occtype if $occtype >= 1 & $occtype <= 9 
