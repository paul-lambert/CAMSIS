...
****  PAUL LAMBERT, UNIV. STIRLING, 20 AUG 2011
...
** ADAPTED TO FRENCH OCCUPATIONAL DATA (MARRIAGE/COHABITING COMBINATIONS), FROM IPUMS-I. 
...
*        relevant occ codes plus a variable which has (or should have) a positive correlation to the scales 




** [5] OCC template file (PATHE) 
*      This is specific to the derivation macros. 
...
*      For FR 1962 there are four templates corresponding to the four source datasets 
...
** The global arguments 'occvar' and 'corvar' are used to name the occ code 
...
capture program drop france_ca

program define france_ca 
...
rename tempnname `occ1' /* two-stage in case names are already shared */
sort `occ1' 
...
* (if not defined a priori by 'pathe', the fall-backs are the most populous occupation in the 1-digit)
* (if education is included, meaning some occs have values 10000+occ, then recodes are done within educ categories)

use  $pathe, clear /* a priori, pathe must have vars called 'occ' and 'occ3' */
gen `occ1'=occ  
rename occ3 isco4
sum 
sort `occ1'
sav $path9\m77.dta, replace
use  $pathe, clear /* a priori, pathe must have vars called 'occ' and 'occ3' */
gen `occ2'=occ  
rename occ3 isco4
sum 
sort `occ2'
sav $path9\m78.dta, replace
...
keep `occ1' cfreq 
sort `occ1'
merge `occ1' using $path9\m77.dta
keep if _merge==1 | _merge==3
drop _merge
gen isco1=floor(`occ1' /1000)
gen isco2=floor(`occ1' /100)
gen isco3=floor(`occ1' /10)
** => isco4 is the a priori recode, if any, as taken from pathe 
** => isco3 is the 1 decimal recode, applicable to most French schemes
** => isco2 is the 2 decimal recode, not usually specified on French data
** => isco1 is the 3 decimal recode, not usually specified on French data
* Find the most common occs within the relevant aggregated units
* 1
capture drop rcount
capture drop rprop
egen rcount=sum(cfreq), by(isco1)
egen rprop=sum(cfreq), by(`occ1') 
replace rprop=rprop / rcount 
gsort +isco1 -rprop
capture drop tocc
gen tocc=`occ1'
replace tocc=0 if (isco1[_n-1]==isco1 )
egen mod1_m=max(tocc), by(isco1)
* 2 
capture drop rcount
capture drop rprop
egen rcount=sum(cfreq), by(isco2)
egen rprop=sum(cfreq), by(`occ1') 
replace rprop=rprop / rcount 
gsort +isco2  -rprop
capture drop tocc
gen tocc=`occ1'
replace tocc=0 if (isco2[_n-1]==isco2 )
egen mod2_m=max(tocc), by(isco2)
* 3 
capture drop rcount
capture drop rprop
egen rcount=sum(cfreq), by(isco3)
egen rprop=sum(cfreq), by(`occ1') 
replace rprop=rprop / rcount 
gsort +isco3 -rprop
capture drop tocc
gen tocc=`occ1'
replace tocc=0 if (isco3[_n-1]==isco3 )
egen mod3_m=max(tocc), by(isco3)
* 4 
capture drop rcount
capture drop rprop
egen rcount=sum(cfreq), by(isco4)
egen rprop=sum(cfreq), by(`occ1') 
replace rprop=rprop / rcount 
gsort +isco4 -rprop
capture drop tocc
gen tocc=`occ1'
replace tocc=0 if (isco4[_n-1]==isco4 )
egen mod4_m=max(tocc), by(isco4)
* 
codebook `occ1' mod1_m mod2_m mod3_m mod4_m , compact
keep `occ1' mod1_m mod2_m mod3_m mod4_m 
sort `occ1'
egen tagi=tag(`occ1')
tab tagi
keep if tagi==1
...
keep `occ2' cfreq 
sort `occ2'
merge `occ2' using $path9\m78.dta
keep if _merge==1 | _merge==3
drop _merge
gen isco1=floor(`occ2' /1000)
gen isco2=floor(`occ2' /100)
gen isco3=floor(`occ2' /10)
** => isco4 is the a priori recode, if any, as taken from pathe 
** => isco3 is the 1 decimal recode, applicable to most French schemes
** => isco2 is the 2 decimal recode, not usually specified on French data
** => isco1 is the 3 decimal recode, not usually specified on French data
...
gen tocc=`occ2'
replace tocc=0 if (isco4[_n-1]==isco4 )
egen mod4_f=max(tocc), by(isco4)
* 
codebook `occ2' mod1_f mod2_f mod3_f mod4_f , compact
keep `occ2' mod1_f mod2_f mod3_f mod4_f 
sort `occ2'
egen tagi=tag(`occ2')
tab tagi
keep if tagi==1
...
** 4digit level recodes (only ordinarily a priori recodes) 

sort `occ1'
capture drop n_occ1
egen n_occ1=sum(cfreq2), by(`occ1')
sort `occ2'
capture drop n_occ2
egen n_occ2=sum(cfreq2), by(`occ2')
...
**** [THIS OPTION WON'T BE USED IN FRENCH DATA ANALYSIS]  
...
**  Next, distribute scores to all known occ units, with the 'orig' indicator to show if isco was represented in 
*     version-specific derivation
* (The commands below will calculate group isco scores as weighted means, then link these with template data if necessary) 
...
* Add value labels if held
capture do $labeldofile
capture label values $occtype $labelitem 
numlabel _all, add

drop if missing($occtype)
codebook, compact
summarize
sav $pathd_details, replace
